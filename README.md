...
| `list_assignments` | Assignments with due dates **and submission status** ('graded' / 'submitted' / 'not submitted'); scope by `sem`, `course_ids`, or `due_within_days` (e.g. 7 = due this week). | `mod_assign_get_assignments` + `mod_assign_get_submission_status` (per assignment, ≤5 in parallel) |
